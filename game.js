...
function create() {
    ...
    this.time.addEvent({
        ...
        callback: () => {
            ...
            ice.setScale(0.5);
            ice.setTint(0x99ffff); // light blue
            ice.melt = 100;
            ice.setBounce(0.5);
    ...
    // Cup bars (horizontal + vertical for cage)
    const barCount = 6;
    const barWidth = 50;
    ...
    for (let i = 0; i < 2; i++) {
        let bar = this.physics.add.staticImage(300 + i*250, 460, 'cup_bar');
    ...
    // Collide ice cubes with cup bars
    cupBars.forEach(bar => {
